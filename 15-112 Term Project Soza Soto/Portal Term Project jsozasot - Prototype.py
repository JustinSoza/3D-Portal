...
'''
Horizaontal Momentum
    - Velocity in = velocity out
    - If velocity in <= benchmark, velocity out = benchmark
...
class Player(object):
    def __init__(self, x, y, z, width, height, speed, start, restingY, health):
        self.x = x
        self.y = y
        self.z = z
        self.width = width
...
class Enemy(object):
    def __init__(self, x, y, z, w, h, degrees, active, alive, sight):
        self.x = x
        self.y = y
        self.z = z
        self.w = w
...
def resetLevel(app):
    if app.level == 1:
        # INitialize Enemy
        app.enemies = set()
        app.enemies.add(Enemy(-100, 30, 30, 20, 30, 180, False, True, [0, 0, 0]))
        app.enemyPoints = dict()
        # Initialize walls
        app.walls = []
        app.walls.append( createWall(app, -300, 150, -400, 100, 150, 50) )
        app.walls.append( createWall(app, -300, 50, -200, 100, 50, 150) )
        app.wallPoints = dict()
        # Initialize boxes
        ...
        health = 100
        app.player = Player(x, y, z, w, h, 0, time.time(), h, health)
        closestPlayerClosestFace(app)
        app.playerPoints = []
...
def mousePressed(app,event):
    ...
    sol = detectWall(app, app.player, app.raycastCoords)
    facing = None
    if sol != None:
        x, y, z, facing = sol
        app.rayCastCoords = [x, y, z]
...
def closestPlayerClosestFace(app):
    player = app.player
    topDistance = getDistance(app, numpy.array([player.x, player.y + player.height, player.z]), app.cameraCoords)
    frontDistance = getDistance(app, numpy.array([player.x, player.y, player.z + player.width]), app.cameraCoords)
    leftDistance = getDistance(app, numpy.array([player.x - player.width, player.y, player.z]), app.cameraCoords)
    rightDistance = getDistance(app, numpy.array([player.x + player.width, player.y, player.z]), app.cameraCoords)
    backDistance = getDistance(app, numpy.array([player.x, player.y, player.z - player.width]), app.cameraCoords)
    bottomDistance = getDistance(app, numpy.array([player.x, player.y - player.height, player.z]), app.cameraCoords)
...
def findLegalPositionE(app, start, end):
    x1, x2 = start[0], end[0]
    y1, y2 = start[1], end[1] - .0000001
    z1, z2 = start[2], end[2]
    xBound = -500
    yBound = 0
    zBound = -500
...
def detectPlayer(app, start, end):
    player = app.player
    x1, y1, z1 = start[0], start[1], start[2]
    x2, y2, z2 = end[0], end[1], end[2]
    closest = None
    ############################### X FACING playerS #########################
    xBound = player.x + player.width
    t = (xBound - x1) / (x2 - x1)
    y = y1 + (y2 - y1) * t
    z = z1 + (z2 - z1) * t
    if z >= (player.z - player.width) and z <= (player.z + player.width) and t >= 0:
        distance = getDistance(app, [x1, 0, y1], [player.x + player.width, 0, z])
        if closest == None or distance < closest: 
            if y <= player.y + player.height and y >= player.height - player.height:
                closest = distance
                projX, projZ, projY = x1 + (x2 - x1) * t, z, y
    xBound = player.x - player.width
    t = (xBound - x1) / (x2 - x1)
    ...
    if z >= (player.z - player.width) and z <= (player.z + player.width) and t >= 0:
        ...
        if closest == None or distance < closest: 
            if y <= player.y + player.height and y >= player.height - player.height:
                ...
                projX, projZ, projY = x1 + (x2 - x1) * t, z, y
    ################################ Z FACING playerS ########################
    zBound = player.z + player.width
    t = (zBound - z1) / (z2 - z1)
    x = x1 + (x2 - x1) * t
    if x >= (player.x - player.width) and x <= (player.x + player.width) and t >= 0:
        distance = getDistance(app, [x1, 0, y1], [x, 0, player.z + player.width])
        if closest == None or distance < closest: 
            if y <= player.y + player.height and y >= player.height - player.height:
                closest = distance
                projX, projZ, projY = x, z1 + (z2 - z1) * t, y
    ...
    if x >= (player.x - player.width) and x <= (player.x + player.width) and t >= 0:
        ...
        if closest == None or distance < closest: 
            if y <= player.y + player.height and y >= player.height - player.height:
                projX, projZ, projY = x, z1 + (z2 - z1) * t, y
    #################################### Y FACING playerS ####################
    yBound = player.y + player.height
    if player.y > yBound:
        t = (yBound - y1) / (y2 - y1)
        x = x1 + (x2 - x1) * t
        z = z1 + (z2 - z1) * t
        if x <= player.x + player.width and x >= player.x - player.width and z <= player.z + player.width and z >= player.z - player.width:
            projX, projY, projZ = x, yBound, z
            closest = ':)'
    if closest != None: 
...
def updateEnemyProjections(app):
    for enemy in app.enemies:
        dEnemy = dict()
        x, y, z, l, h, w = enemy.x, enemy.y, enemy.z, enemy.w, enemy.h, enemy.w
        # Front Side
        x0, y0 = project(app, [x - l, y + h, z + w])
        x1, y1 = project(app, [x + l, y + h, z + w])
        x2, y2 = project(app, [x - l, y - h, z + w])
        x3, y3 = project(app, [x + l, y - h, z + w])
        dEnemy['front'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        # Top Side
        x0, y0 = project(app, [x - l, y + h, z + w])
        x1, y1 = project(app, [x - l, y + h, z - w])
        x2, y2 = project(app, [x + l, y + h, z + w])
        x3, y3 = project(app, [x + l, y + h, z - w])
        dEnemy['top'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        # Right Side
        x0, y0 = project(app, [x + l, y + h, z + w])
        x1, y1 = project(app, [x + l, y + h, z - w])
        x2, y2 = project(app, [x + l, y - h, z + w])
        x3, y3 = project(app, [x + l, y - h, z - w])
        dEnemy['right'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        # Bottom Side
        x0, y0 = project(app, [x - l, y - h, z + w])
        x1, y1 = project(app, [x - l, y - h, z - w])
        x2, y2 = project(app, [x + l, y - h, z + w])
        x3, y3 = project(app, [x + l, y - h, z - w])
        dEnemy['bottom'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        # Left Side
        x0, y0 = project(app, [x - l, y + h, z + w])
        x1, y1 = project(app, [x - l, y + h, z - w])
        x2, y2 = project(app, [x - l, y - h, z + w])
        x3, y3 = project(app, [x - l, y - h, z - w])
        dEnemy['left'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        # Back Side
        x0, y0 = project(app, [x - l, y + h, z - w])
        x1, y1 = project(app, [x - l, y - h, z - w])
        x2, y2 = project(app, [x + l, y + h, z - w])
        x3, y3 = project(app, [x + l, y - h, z - w])
        dEnemy['back'] = [x0, y0, x1, y1, x2, y2, x3, y3]
        app.enemyPoints[enemy] = dEnemy
...
def enemyScan(app):
    ...
    for enemy in app.enemies:
        if enemy.alive:
            ...
            enemyCoords = numpy.array([x, y, z])
            u = numpy.array([[1],[0]])
            angle = enemy.degrees
            rotate = numpy.array([[math.cos(angle), math.sin(angle)],
                        [(-1) * math.sin(angle), math.cos(angle)]])
            v = numpy.multiply(u, rotate)
            w = numpy.array([v[0][0] + x, y, v[1][0] + z])
            projectedEnemy = findLegalPositionE(app, enemyCoords, w)
            enemy.sight = projectedEnemy
            wallSight = detectWall(app, enemyCoords, projectedEnemy)
            if wallSight != None:
                print('Wall detected')
                enemy.sight = wallSight
                projectedEnemy = wallSight
            playerSight = detectPlayer(app, enemyCoords, projectedEnemy)
            if playerSight != None:
                print('Player detected')
                enemy.sight = playerSight
                enemy.active = True
            else:
                enemy.active = False
...
def drawWalls(app, canvas):
    for wall in app.walls:
        ...
        if app.front:
            ...
            canvas.create_polygon(x0, y0, x1, y1, x2, y2, x3, y3, fill = 'gray90',
            outline = 'black')
        else:
            x1, y1, x0, y0, x2, y2, x3, y3 = d['back']
            canvas.create_polygon(x0, y0, x1, y1, x2, y2, x3, y3, fill = 'gray10',
            outline = 'black')
        if app.top:
        ...
        if app.right:
            ...
            canvas.create_polygon(x0, y0, x1, y1, x2, y2, x3, y3, fill = 'gray70',
            outline = 'black')
...
def drawButtons(app, canvas):
    ...
    for bottom, top in app.buttonPoints:
        ...
        counter += 1
        if counter > 0:
            topColor = 'dodgerblue2'
            bottomColor = 'blue4'
            frontColor = 'dodgerblue4'
            backColor = 'dodgerblue4'
            rightColor = 'dodgerblue3'
            leftColor = 'RoyalBlue4' 
            outline = 'blue'
        # BOTTOM
        ...
        # Top
        if app.top:
            canvas.create_polygon(x4, y4, x5, y5, x6, y6, x7, y7, fill = '', outline = outline)
        # Bottom
        else:
            canvas.create_polygon(x0, y0, x1, y1, x2, y2, x3, y3, fill = '', outline = outline)
        # Front
        if app.front:
            canvas.create_polygon(x3, y3, x2, y2, x6, y6, x7, y7, fill = '', outline = outline)
        # Back
        else:
            canvas.create_polygon(x0, y0, x4, y4, x5, y5, x1, y1, fill = '', outline = outline)
        # Right
        if app.right:
            canvas.create_polygon(x1, y1, x2, y2, x6, y6, x5, y5, fill = '', outline = outline)
        # Left
        else:
            canvas.create_polygon(x3, y3, x0, y0, x4, y4, x7, y7, fill = '', outline = outline)
...
def drawPain(app, canvas):
    player = app.player
    if player.health > 10:
        canvas.create_text(app.width // 2 - 30, 25, text = player.health, font = 'Lato 10')
    else:
        canvas.create_text(app.width // 2 - 30, 25, text = player.health, font = 'Lato 10', fill = 'red')
    ### Draw Heart ###
    heartcx, heartcy = app.width // 2, 20
    r = 5
    cx0, cx1 = heartcx - r, heartcx + r 
    canvas.create_oval(cx0 - r, heartcy - r, cx0 + r, heartcy + r,
    fill = 'red', outline = '')
    canvas.create_oval(cx1 - r, heartcy - r, cx1 + r, heartcy + r, 
    fill = 'red', outline = '')
    canvas.create_polygon(heartcx, heartcy + (3 * r), cx0 - r - 1, heartcy, cx1 + r + 1, heartcy,
    fill = 'red', outline = '')
...
def drawEnemySight(app, canvas):
    for enemy in app.enemies:
        if enemy.sight != None:
            x0, y0 = project(app, [enemy.x, enemy.y, enemy.z])
            x1, y1 = project(app, enemy.sight)
            canvas.create_line(x0, y0, x1, y1, fill = 'red')

def drawEnemies(app, canvas):
    for enemy in app.enemies:
        d = app.enemyPoints[enemy]
        if app.front:
            x1, y1, x0, y0, x2, y2, x3, y3 = d['front']
            canvas.create_polygon(x0, y0, x1, y1, x2, y2, x3, y3, fill = '',
            outline = 'black')
