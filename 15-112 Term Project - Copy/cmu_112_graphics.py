...
import datetime
MAJOR_VERSION = 0
MINOR_VERSION = 8.8 # version 0.8.8
LAST_UPDATED  = datetime.date(year=2021, month=2, day=25)
...
class App(object):
    ...
    def rightClickPressed(app, event): pass # use event.x and event.y
    ...
    def _mousePressedWrapper(app, event):
        ...
        if ((event.x < 0) or (event.x > app.width) or
            ...
            if (app._methodIsOverridden('mousePressed')):
                ...
                app._redrawAllWrapper()
    @_safeMethod
    def _rightClickPressedWrapper(app, event):
        if (not app._running) or app._paused: return
        if ((event.x < 0) or (event.x > app.width) or
            (event.y < 0) or (event.y > app.height)):
            app._mousePressedOutsideWindow = True
            ...
            app._mousePressedOutsideWindow = False
            app._rightClickIsPressed = True
            app._lastRightClickMousePosn = (event.x, event.y)
            if (app._methodIsOverridden('rightClickPressed')):
                event = App.MouseEventWrapper(event)
                app.rightClickPressed(event)
                app._redrawAllWrapper()
    ...
    def _mouseReleasedWrapper(app, event):
        ...
        app._rightClickIsPressed = False
    ...
    def run(app):
        ...
        app._lastRightClickMousePosn = (-1, -1)
        ...
        if (App._theRoot is None):
            ...
            App._theRoot.bind("<Button-3>", lambda event: App._theRoot.app._rightClickPressedWrapper(event))
...
class TopLevelApp(App):
    ...
    def rightClickPressed(app, event): app._callFn('rightClickPressed', app, event)
